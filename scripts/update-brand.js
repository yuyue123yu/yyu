...
async function main() {
  ...
  const newCompanyName = await question(
    '请输入新的公司全称（例如：法律通有限公司）: ',
  )
  const newEmailDomain = await question(
    '请输入新的邮箱域名（例如：falvtong.com）: ',
  )
  const newRegistrationNumber = await question(
    '请输入新的公司注册号（可选，按回车跳过）: ',
  )
  ...
  if (newRegistrationNumber.trim()) {
    console.log(
      `  ${CURRENT_BRAND.registrationNumber} → ${newRegistrationNumber}`,
    )
  }
  ...
  console.log('\n✨ 更新完成！')
  console.log(
    `📊 统计：更新了 ${updatedFiles} 个文件，共 ${totalReplacements} 处替换\n`,
  )
  console.log('⚠️  注意事项：')
