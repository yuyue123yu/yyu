      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 mb-2 md:mb-3">
          ...
          <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-lg md:rounded-xl p-3 md:p-4 border-2 border-red-300 relative overflow-hidden">
            ...
            <h3 className="font-bold text-neutral-900 mb-0.5 text-xs md:text-sm">
              {t('home.legalTemplates')}
            </h3>
            <p className="text-xs text-neutral-700 mb-1.5 md:mb-2">
          ...
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg md:rounded-xl p-3 md:p-4 border-2 border-purple-300">
            ...
            <p className="text-xs text-neutral-700 mb-1.5 md:mb-2">
              RM 299 {t('home.reviewFrom')}{' '}
              <span className="font-bold text-purple-600">
                {t('home.professionalReview')}
              </span>
            </p>
            <Link
              href="/review"
          ...
          <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg md:rounded-xl p-3 md:p-4 border-2 border-amber-300">
            ...
            <h3 className="font-bold text-neutral-900 mb-0.5 text-xs md:text-sm">
              {t('home.legalKnowledgeBase')}
            </h3>
            <p className="text-xs text-neutral-700 mb-1.5 md:mb-2">
              1000+ {t('home.legalArticles')}{' '}
            ...
            {[
              {
                rank: 1,
                name: 'Ahmad Abdullah',
                specialty: t('services.business'),
                sales: '156',
              },
              {
                rank: 2,
                name: 'Sarah Wong',
                specialty: t('services.family'),
                sales: '189',
              },
              {
                rank: 3,
                name: 'Kumar Rajesh',
                specialty: t('services.property'),
                sales: '178',
              },
              {
                rank: 4,
              ...
              <div
                key={item.rank}
                className="bg-white/10 backdrop-blur-sm rounded-lg p-2 md:p-3 border border-white/20"
              >
                <div className="text-lg md:text-xl font-bold text-accent-400 mb-0.5 md:mb-1">
                  #{item.rank}
                </div>
                <p className="font-semibold text-xs mb-0.5">{item.name}</p>
                <p className="text-xs text-blue-100 mb-0.5 md:mb-1">
                  {item.specialty}
                </p>
                <p className="text-xs text-accent-300 font-bold">
                  {item.sales} {t('home.consultations')}
                </p>
              </div>
            ))}
          </div>
