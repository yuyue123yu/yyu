...
function AdminLayoutContent({ user, profile, children }: AdminLayoutClientProps) {
  ...
  const menuGroups = [
    ...
    },
    {
      title: '网站设置',
      items: [
        { icon: Palette, label: '品牌设置', href: '/admin/branding' },
              ...
              <div className="space-y-1">
                {group.items.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="flex items-center gap-3 px-4 py-2.5 text-neutral-700 hover:bg-primary-50 hover:text-primary-700 rounded-lg transition-all text-sm"
                  >
